...
async def query_documents(
    ...
    if isinstance(rag_service, PrismRAGService) and request.mode != "vector_only":
...
async def process_document(
    ...
    if document.status in (
        ...
        timeout = settings.PRISMRAG_PROCESSING_TIMEOUT_MINUTES
...
async def process_batch(
    ...
    if accepted_ids:
        ...
        asyncio.get_event_loop().create_task(_process_batch_background(accepted_ids))
...
async def reindex_document(
    ...
    """Re-process an existing document through the PrismRAG pipeline."""
    result = await db.execute(select(Document).where(Document.id == document_id))
    document = result.scalar_one_or_none()
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            f"Failed to delete old data for reindex: {e}"
        )
    ...
    try:
        chunk_count = await rag_service.process_document(
            document_id=document_id, file_path=str(file_path)
        )
        return DocumentProcessResponse(
            document_id=document_id,
            status=DocumentStatus.INDEXED.value,
            chunk_count=chunk_count,
            message=f"Re-indexed with PrismRAG: {chunk_count} chunks created",
        )
    except Exception as e:
        raise HTTPException(
...
async def get_workspace_rag_stats(
    ...
    # Count PrismRAG documents (parser_version = 'docling')
    prismrag_result = await db.execute(
        select(func.count(Document.id)).where(
            Document.workspace_id == workspace_id, Document.parser_version == "docling"
        )
    )
    prismrag_documents = prismrag_result.scalar() or 0
    ...
    return ProjectRAGStatsResponse(
        ...
        prismrag_documents=prismrag_documents,
    )
...
async def _get_kg_service(workspace_id: int):
    """Get KnowledgeGraphService for a knowledge base (if PrismRAG is active)."""
...
@router.get(
    "/relationships/{workspace_id}", response_model=list[KGRelationshipResponse]
)
async def get_kg_relationships(
    ...
    try:
        ...
        logger.error(
            f"Failed to get KG relationships for workspace {workspace_id}: {e}"
        )
        return []
...
async def get_workspace_analytics(
    ...
    # KG analytics (optional — only if PrismRAG active)
    kg_analytics = None
    if prismrag_documents > 0:
        try:
            kg = await _get_kg_service(workspace_id)
            ...
            kg_analytics = KGAnalyticsResponse(
                ...
                top_entities=[
                    KGEntityResponse(**e) for e in analytics_data["top_entities"]
                ],
                avg_degree=analytics_data["avg_degree"],
            )
        except Exception as e:
            logger.warning(
                f"Failed to get KG analytics for workspace {workspace_id}: {e}"
            )
...
# ---------------------------------------------------------------------------
# Chat endpoint — LLM-powered document Q&A via PrismRAG
# ---------------------------------------------------------------------------
...
async def chat_with_documents(
    ...
    if isinstance(rag_service, PrismRAGService):
        result = await rag_service.query_deep(
    ...
    if kg_summary:
        try:
            ...
            for lower_name, original_name in entity_names.items():
                if len(lower_name) >= 2 and (
                    lower_name in answer_lower or lower_name in context_lower
                ):
    ...
    try:
        ...
        assistant_row = ChatMessageModel(
            ...
            image_refs=[img.model_dump() for img in chat_image_refs]
            if chat_image_refs
            else None,
