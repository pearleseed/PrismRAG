...
class BaseDocumentParser(ABC):
    ...
    # Subclasses set this so prism_rag_service can store the parser name.
    ...
    def _caption_images(self, images: list[ExtractedImage]) -> None:
        ...
        for img in images:
            ...
            try:
                ...
                if result_str:
                    img.caption = " ".join(result_str.strip().split())[:500]  # ty:ignore[unresolved-attribute]
    ...
    def _caption_tables(self, tables: list[ExtractedTable]) -> None:
        ...
        for tbl in tables:
            ...
            try:
                ...
                if len(table_md) > settings.PRISMRAG_MAX_TABLE_MARKDOWN_CHARS:
                    table_md = (
                        table_md[: settings.PRISMRAG_MAX_TABLE_MARKDOWN_CHARS]
                        + "\n... (truncated)"
                    )
                ...
                if result_str:
                    tbl.caption = " ".join(result_str.strip().split())[:500]  # ty:ignore[unresolved-attribute]
            except Exception as e:
