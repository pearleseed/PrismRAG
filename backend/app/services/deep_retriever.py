...
Pipeline:
  ...
  2. Vector search → over-fetch top-N candidates (PRISMRAG_VECTOR_PREFETCH)
  3. Cross-encoder rerank → precision filter to top-K (PRISMRAG_RERANKER_TOP_K)
...
class DeepRetriever:
    ...
    async def query(
        ...
        Flow:
          1. [parallel] KG query + Vector over-fetch (PRISMRAG_VECTOR_PREFETCH)
        ...
        prefetch_k = max(settings.PRISMRAG_VECTOR_PREFETCH, top_k * 3)
    ...
    def _rerank_chunks(
        ...
        reranked = self.reranker.rerank(
            ...
            documents=doc_texts,
            top_k=top_k,
            min_score=settings.PRISMRAG_MIN_RELEVANCE_SCORE,
        )
        ...
        if not reranked:
            ...
            logger.warning(
                ...
                f"{settings.PRISMRAG_MIN_RELEVANCE_SCORE}, falling back to top 3"
            )
