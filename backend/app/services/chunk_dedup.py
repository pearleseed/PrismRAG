...
def filter_noise(chunks: list[EnrichedChunk]) -> list[EnrichedChunk]:
    ...
    """
    min_len = settings.PRISMRAG_DEDUP_MIN_CHUNK_LENGTH
    ...
    if removed:
        logger.info(
            f"Noise filter: removed {removed}/{len(chunks)} boilerplate/short chunks"
        )
...
def dedup_near(
    ...
    if threshold is None:
        threshold = settings.PRISMRAG_DEDUP_NEAR_THRESHOLD
...
def deduplicate_chunks(
    ...
    """
    if not settings.PRISMRAG_DEDUP_ENABLED:
        return chunks, {
            "input": len(chunks),
            "output": len(chunks),
            "noise_removed": 0,
