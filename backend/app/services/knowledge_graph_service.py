...
async def _kg_llm_complete(
    ...
    **kwargs,
    ...
    # acomplete() returns str | LLMResult; LightRAG expects a plain str.
    if hasattr(result, "content"):
        return str(result.content)  # type: ignore[union-attr]
    return str(result)
...
class KnowledgeGraphService:
    ...
    def __init__(
        ...
        self.kg_language = kg_language or settings.PRISMRAG_KG_LANGUAGE
        self.kg_entity_types = kg_entity_types or settings.PRISMRAG_KG_ENTITY_TYPES
        self._rag = None
    ...
    async def _get_rag(self):
        ...
        self._rag = LightRAG(
            ...
            chunk_token_size=settings.PRISMRAG_KG_CHUNK_TOKEN_SIZE,
    ...
    async def query(
        ...
        try:
            result = await asyncio.wait_for(
                rag.aquery(
                    question,
                    param=QueryParam(mode=mode, top_k=top_k),  # type: ignore[arg-type]  # ty:ignore[invalid-argument-type]
                ),
                timeout=settings.PRISMRAG_KG_QUERY_TIMEOUT,
            )
            return result or ""
        except asyncio.TimeoutError:
            logger.warning(
                f"KG query timed out after {settings.PRISMRAG_KG_QUERY_TIMEOUT}s "
                f"for workspace {self.workspace_id}"
            )
            return ""
        except Exception as e:
    ...
    async def cleanup(self) -> None:
        ...
        if self._rag:
            try:
                ...
                logger.warning(
                    f"KG cleanup failed for workspace {self.workspace_id}: {e}"
                )
            self._rag = None
            self._initialized = False
    ...
    async def get_entities(
        ...
        try:
            ...
            logger.error(
                f"Failed to get KG nodes for workspace {self.workspace_id}: {e}"
            )
            return []
    ...
    async def get_relationships(
        ...
        try:
            ...
            logger.error(
                f"Failed to get KG edges for workspace {self.workspace_id}: {e}"
            )
            return []
    ...
    async def get_graph_data(
        ...
        try:
            ...
            logger.error(
                f"Failed to get KG graph for workspace {self.workspace_id}: {e}"
            )
    ...
    async def get_relevant_context(
        ...
        try:
            ...
            logger.error(
                f"Failed to get raw KG data for workspace {self.workspace_id}: {e}"
            )
            return ""
    ...
    async def get_analytics(self) -> dict:
        ...
        try:
            ...
            logger.error(
                f"Failed to get KG analytics for workspace {self.workspace_id}: {e}"
            )
            return {
                "entity_count": 0,
