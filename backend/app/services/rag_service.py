...
class RAGService:
    ...
    async def process_document(self, document_id: int, file_path: str) -> int:
        ...
        try:
            ...
            if not chunks:
                ...
                await self.db.commit()
                logger.warning(
                    f"Document {document_id} produced no chunks (empty content)"
                )
                return 0
    ...
    def query(
        self, question: str, top_k: int = 5, document_ids: list[int] | None = None
    ) -> RAGQueryResult:
        """
        ...
        results = self.vector_store.query(
            query_embedding=query_embedding, n_results=top_k, where=where
        )
        ...
        return RAGQueryResult(chunks=chunks, context=context, query=question)
