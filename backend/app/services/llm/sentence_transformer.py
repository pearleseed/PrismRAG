...
class SentenceTransformerEmbeddingProvider(EmbeddingProvider):
    ...
    def model(self):
        if self._model is None:
            ...
            logger.info(
                "Loading sentence-transformers KG embedding model: %s", self._model_name
            )
            self._model = SentenceTransformer(self._model_name)
            self._dimension = self._model.get_embedding_dimension() or _KNOWN_DIMS.get(
                self._model_name, 1024
            )
            logger.info(
