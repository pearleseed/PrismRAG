    ...
    def _to_contents(self, messages: list[LLMMessage]) -> list[types.Content]:
        ...
        for msg in messages:
            ...
            if msg._raw_provider_content is not None:
                contents.append(msg._raw_provider_content)  # type: ignore[arg-type]  # ty:ignore[invalid-argument-type]
                continue
    ...
    def _build_thinking_config(self) -> types.ThinkingConfig:
        ...
        if self._major_version >= 3:
            return types.ThinkingConfig(
                thinking_level=self._thinking_level,  # type: ignore[arg-type]  # ty:ignore[invalid-argument-type]
        ...
        *,
        ...
        try:
            response = self._client.models.generate_content(
                model=self._model,
                contents=contents,  # type: ignore[arg-type]  # ty:ignore[invalid-argument-type]
                config=config,
            )
            if use_think:
        ...
        for i in range(0, len(texts), self._BATCH_SIZE):
            ...
            try:
                result = self._client.models.embed_content(
                    model=self._model,
                    contents=batch,  # type: ignore[arg-type]  # ty:ignore[invalid-argument-type]
                )
                for emb in result.embeddings or []:
                    all_embeddings.append(emb.values or [])
            except Exception as e:
