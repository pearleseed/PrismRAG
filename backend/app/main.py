"""
PrismRAG — standalone Knowledge Base + RAG application.
"""
...
async def lifespan(app: FastAPI):
    logger.info("Starting PrismRAG API...")
    import os
    ...
    if auto_create:
        async with engine.begin() as conn:
            ...
            if not is_initialized:
                ...
                if os.path.exists(schema_path):
                    ...
                    await conn.execute(
                        text(
                            "INSERT INTO public.alembic_version (version_num) VALUES ('2047460692d0') ON CONFLICT DO NOTHING;"
                        )
                    )
                else:
                    await conn.run_sync(Base.metadata.create_all)
                    logger.info(
                        "Database tables created/verified (Base.metadata.create_all)"
                    )
            else:
        ...
        async with AsyncSession(engine) as session:
            ...
            if stale_ids:
                ...
                logger.warning(
                    f"Recovered {len(stale_ids)} stale documents: {stale_ids}"
                )
    else:
...
app = FastAPI(
    ...
    description="PrismRAG — Knowledge Base with semantic search, knowledge graph, and LLM chat",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc",
...
# OpenAI-compatible surface for external UIs (e.g. Open WebUI) — /v1/models, /v1/chat/completions
from app.api.openai_compat import router as openai_compat_router  # noqa: E402

app.include_router(openai_compat_router)

# Static files — document images extracted by PrismRAG (Docling)
